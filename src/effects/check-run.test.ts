...
import { assembleReleaseReport, type AssembleReleaseReportInput } from "../domain/release/report.js";
...
describe("releaseToCheck rendering", () => {
  it("renders coverage before findings from the persisted report", () => {
    ...
    const decision = evaluateRelease([], POLICY, COMPLETE_COVERAGE, undefined, { required: true, complete: false });
  ...
  it("carries candidate/base identity, report id/version, policy and reasons", () => {
    const report = assembleReleaseReport(baseInput({ decision: evaluateRelease([], POLICY, COMPLETE_COVERAGE, undefined, { required: true, complete: true }) }));
  ...
  it("never titles an incomplete-coverage ship as clean (defence-in-depth)", () => {
    ...
    const report = assembleReleaseReport(
      baseInput({
        decision: evaluateRelease([], POLICY, COMPLETE_COVERAGE),
        candidateCi: { required: true, applicable: true, status: "partial", observations: [], gaps: ["ci/build missing"] },
      }),
    );
  ...
  it("surfaces every unresolved model risk with its citation", () => {
    ...
    const report = assembleReleaseReport(
      baseInput({
        decision: evaluateRelease(finding, POLICY, COMPLETE_COVERAGE, { retainedRiskCount: 1, complete: true }, { required: true, complete: true }),
        releaseRisk: {
  ...
  it("keeps the conclusion neutral for a confirmed stop", () => {
    ...
    const report = assembleReleaseReport(baseInput({ findings: [finding], decision: evaluateRelease([finding], POLICY, COMPLETE_COVERAGE) }));
