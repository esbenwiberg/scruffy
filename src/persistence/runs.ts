...
import { isCompleteReview, type NightlyReport, type NightlyWorkGraph, type NightlyWorkItem } from "../domain/findings/work-graph.js";
  ...
  async #writeReport(client: PoolClient, params: CommitNightlyDecisionParams, now: Date): Promise<void> {
    ...
    for (const finding of report.findings) {
      ...
      if (proposal !== null) {
        ...
        // re-commit must never reset a proposal that has already been published.
        await client.query(
